...
export class ReplayAnalyzer {
  private actorToPlayer: PlayerMapping = {};
  ...
  exploreJsonStructure(
    ...
    if (typeof data === "object" && data !== null) {
      if (Array.isArray(data)) {
        console.log(
          `${"  ".repeat(currentDepth)}[List] ${path} (length: ${data.length})`
        );
        if (data.length > 0 && currentDepth < maxDepth - 1) {
          this.exploreJsonStructure(
            data[0],
            `${path}[0]`,
            maxDepth,
            currentDepth + 1
        ...
        Object.entries(data).forEach(([key, value]) => {
          ...
          if (typeof value === "object" && value !== null) {
            ...
            console.log(
              `${"  ".repeat(currentDepth + 1)}Type: ${typeof value}`
            );
          }
        });
      }
  ...
  buildActorMappings(replayData: any): void {
    ...
    console.log(
      `Found ${Object.keys(this.actorToPlayer).length} player mappings`
    );
  }
  ...
  printDemolitionSummary(events: DemolitionEvent[]): void {
    ...
    Object.entries(demoCounts)
      ...
      .forEach(([attacker, count]) => {
        console.log(
          `* ${attacker}: ${count} demolition${count !== 1 ? "s" : ""}`
        );
      });
  }
  ...
  getDemolitionEvents(content: string): DemolitionEvent[] {
    try {
      const replayData = JSON.parse(content);
      this.buildActorMappings(replayData);
      return this.findDemolitions(replayData);
    } catch (error) {
      console.error("Error analyzing replay content:", error);
      return [];
    }
